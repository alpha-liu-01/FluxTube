...
void AudioController::open_mixer()
{
    ...
    static const char *names[] = {
        "Master", "PCM", "Speaker", "Headphone", "Digital", "Playback", nullptr};
    for (const char **name = names; *name != nullptr; ++name)
    {
        snd_mixer_elem_t *elem = find_mixer_element(*name);
        if (elem)
        {
    ...
    master_element_ = first_playback_element();
    if (!master_element_)
    {
        snd_mixer_close(mixerHandle_);
        mixerHandle_ = nullptr;
...
snd_mixer_elem_t *AudioController::find_mixer_element(const std::string &channel) const
{
    if (!mixerHandle_)
    {
        return nullptr;
    ...
    return snd_mixer_find_selem(mixerHandle_, sid);
}

snd_mixer_elem_t *AudioController::first_playback_element() const
{
    if (!mixerHandle_)
    {
        return nullptr;
    }
    for (snd_mixer_elem_t *elem = snd_mixer_first_elem(mixerHandle_); elem != nullptr;
...
{
    snd_mixer_elem_t *elem = find_mixer_element(channel);
    if (!elem)
    {
